    ...
    E: core::fmt::Debug,
{
    ...
    pub fn new(i2c: I2c, addr: u8, mode: &Mode) -> Result<Self, Error<E>> {
        ...
        match s.addr {
            //// Modified ////
            ADDRESS_BASE_1 => {
                let _ = s.i2c.write(0x00, &[0xFF]);
            } //The result is not used because it  will be an I2C error, because theses commands reset sensor and set-up
            ADDRESS_BASE_2 => {
                let _ = s.i2c.write(0x00, &[0x00]);
            } //its address, and the sensor do not respond with Ack-bit, so i2c method will interpret it as an error.
    ...
    pub fn read(&mut self) -> Result<Values, Error<E>> {
        //// Can not really be used because Values's field are private.... ////
